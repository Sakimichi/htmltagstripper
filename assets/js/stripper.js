...
$('#convert').on('click',function()
{
		...
		//$('#convert i').css({'-webkit-animation':'converting 1s linear','animation':'converting 1s linear','-o-animation':'converting 1s linear','-moz-animation':'converting 1s linear','-ms-animation':'converting 1s linear'});
                $('#convert i').addClass('fa-spin')
...
//		var body = $('#test').html();
		...
		if($('#Output').val()=='')
		{
			...
			$('#Output').fadeOut("10000");
                        $('#output-wrap').css({'display':'none','opacity':'0'});
			$('#output-wrap').fadeOut("10000");
		...
		else
		{
			...
			$('#Output').fadeIn("10000");
                        $('#output-wrap').css({'display':'block','opacity':'1'});
			$('#output-wrap').fadeIn("10000");
		}
...
function reset()
{
		//$('#convert i').css({'-webkit-animation':'none','animation':'none','-o-animation':'none','-moz-animation':'none','-ms-animation':'none'});
                $('#convert i').removeClass('fa-spin')
